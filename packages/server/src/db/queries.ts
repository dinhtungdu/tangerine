...
export function createTask(
  ...
  task: Pick<TaskRow, "id" | "project_id" | "source" | "repo_url" | "title"> &
    Partial<Pick<TaskRow, "source_id" | "source_url" | "description" | "user_id" | "branch" | "provider" | "model" | "reasoning_effort" | "type" | "review_pr_number" | "review_task_id">>
): Effect.Effect<TaskRow, DbError> {
  return dbTry(() => {
    const stmt = db.prepare(`
      INSERT INTO tasks (id, project_id, source, source_id, source_url, repo_url, title, description, user_id, branch, provider, model, reasoning_effort, type, review_pr_number, review_task_id)
      VALUES ($id, $project_id, $source, $source_id, $source_url, $repo_url, $title, $description, $user_id, $branch, $provider, $model, $reasoning_effort, $type, $review_pr_number, $review_task_id)
    `)
    stmt.run({
      $id: task.id,
      ...
      $type: task.type ?? "code",
      $review_pr_number: task.review_pr_number ?? null,
      $review_task_id: task.review_task_id ?? null,
    })
