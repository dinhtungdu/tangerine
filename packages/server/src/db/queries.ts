...
export function createTask(
  ...
  task: Pick<TaskRow, "id" | "project_id" | "source" | "title"> &
    Partial<Pick<TaskRow, "source_id" | "source_url" | "type" | "workflow" | "description" | "user_id" | "branch" | "provider" | "model" | "reasoning_effort" | "parent_task_id" | "capabilities">>
): Effect.Effect<TaskRow, DbError> {
  return dbTry(() => {
    const stmt = db.prepare(`
      INSERT INTO tasks (id, project_id, source, source_id, source_url, title, type, workflow, description, user_id, branch, provider, model, reasoning_effort, parent_task_id, capabilities)
      VALUES ($id, $project_id, $source, $source_id, $source_url, $title, $type, $workflow, $description, $user_id, $branch, $provider, $model, $reasoning_effort, $parent_task_id, $capabilities)
    `)
    stmt.run({
      $id: task.id,
