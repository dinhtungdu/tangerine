...
describe("createAcpEventMapper", () => {
  ...
  test("keeps sentence-boundary chunks in one assistant message as paragraphs", () => {
    const mapper = createAcpEventMapper()
    ...
    expect(mapper.mapSessionUpdate({ sessionUpdate: "agent_message_chunk", messageId: "msg-2", content: { type: "text", text: "Second." } }))
      .toEqual([
        { kind: "message.streaming", content: "\n\nSecond.", messageId: "msg-1" },
      ])
    expect(mapper.flushAssistantMessage()).toEqual([{ kind: "message.complete", role: "assistant", content: "First.\n\nSecond.", messageId: "msg-1" }])
  })
