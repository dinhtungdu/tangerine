...
- merge prompt-active `agent_message_chunk` with stable `messageId` into one live assistant message; stream no-`messageId` prose provisionally as narration, persisting it as narration before visible events/sentence boundaries and as assistant when it is the remaining prompt-completion buffer; ignore assistant chunks outside an active prompt turn
