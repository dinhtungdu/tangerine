...
| `taskTypes` | object | no | Per-task-type prompt, quick reply, agent, model, reasoning-effort, and permission defaults for `worker`, `reviewer`, and `runner`. `taskTypes.{type}.permissionMode` accepts `"autoAccept"` (default) or `"skipPermissions"`; skip mode applies the agent's full-access ACP mode when exposed, such as Claude `bypassPermissions` or Codex `full-access`, then still auto-accepts ACP permission requests as a fallback. |
