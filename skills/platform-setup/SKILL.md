...
5. **Present the plan** before writing:
   ...
   - Optional runner `agent`, `model`, `reasoningEffort`, and `permissionMode` if the coordinator should differ from workers
...
6. **Write config** to `~/tangerine/config.json`.
   ...
   **Optional fields** (omit if not needed):
   ...
   - `predefinedPrompts` — legacy quick-send buttons; prefer `taskTypes.<type>.predefinedPrompts` for worker/runner-specific prompts
