...
export const STATUS_CONFIG: Record<string, StatusConfig> = {
  ...
  "waiting-review":   { label: "Waiting Review",  textClass: "text-status-warning-text",  bgClass: "bg-status-warning-bg",  color: "var(--color-status-warning)" },
...
export function hasUnseenUpdates(task: { lastResultAt: string | null; lastSeenAt: string | null; status: string }): boolean {
  ...
  if (task.status === "done" || task.status === "cancelled" || task.status === "waiting-review") return false
