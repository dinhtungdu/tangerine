    ...
    <div className="flex h-[180px] w-full shrink-0 flex-col border-t border-edge @min-[700px]/diff:h-full @min-[700px]/diff:w-[220px] @min-[700px]/diff:border-l @min-[700px]/diff:border-t-0">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="hidden h-11 items-center justify-between px-3 @min-[700px]/diff:flex">
          <span className="text-xs font-semibold text-fg">Changed Files</span>
          <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-surface-secondary px-1.5 font-mono text-2xs font-medium text-fg-muted">
            {files.length}
          </span>
        </div>
        {files.map((file) => {
            ...
            <div key={file.path}>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-md font-medium text-fg">{fileName(file.path)}</div>
                  <div className="hidden truncate text-xxs text-fg-muted @min-[700px]/diff:block">{fileDir(file.path)}</div>
                </div>
                ...
                {fileComments.length > 0 && (
                  <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-fg px-1 text-2xs font-semibold text-surface">
                    {fileComments.length}
                  </span>
                )}
              </button>
                    ...
                    <div key={comment.id} className="rounded-lg border border-edge bg-surface p-3">
                      <div className="mb-1.5 flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs font-medium text-fg">{fileName(comment.filePath)}</span>
                          <span className="rounded bg-surface-secondary px-1.5 py-0.5 text-2xs font-medium text-fg-muted">{comment.lineRef}</span>
                        </div>
                      ...
                      <p className="text-xs leading-relaxed text-fg-muted">{comment.text}</p>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div className="border-t border-edge px-3 py-2 @min-[700px]/diff:px-4 @min-[700px]/diff:py-3">
        <div className="mb-1.5 flex items-center gap-1.5 text-xs text-fg-muted @min-[700px]/diff:mb-2">
          <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-fg px-4 py-2 text-md font-medium text-surface transition hover:bg-fg/90 disabled:opacity-40 @min-[700px]/diff:py-2.5"
        >
