...
const markdownComponents: Components = {
  ...
  code: ({ children, className }) => {
    ...
    if (!className && !insidePre) {
      return <code className="rounded bg-muted px-1 py-0.5 font-mono border border-border not-italic">{children}</code>
    }
...
function SystemPromptMessage({ message }: { message: ChatMessageType }) {
  ...
  return (
    <div className="animate-fade-in overflow-hidden flex flex-col gap-1">
      ...
      >
        <div className="flex h-5 w-5 items-center justify-center rounded-[10px] bg-muted">
        ...
        <svg
          className={`h-3 w-3 text-muted-foreground/50 transition-transform ${expanded ? "rotate-90" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
      ...
      {expanded && (
        <div className="rounded-lg border border-border bg-muted/50 px-3 py-2 text-xs leading-[1.6] text-muted-foreground whitespace-pre-wrap font-mono">
          {message.content}
        </div>
      )}
    </div>
...
export const ChatMessage = memo(function ChatMessage({ message, tasks, onReply, isThinkingActive = false, thinkingDuration }: ChatMessageProps) {
  ...
  if (isSystem) {
    // Short system messages (errors) show inline; long ones (system prompt) show collapsed
    const isLongSystemPrompt = message.content.includes("\n") || message.content.length > 100
    if (isLongSystemPrompt) {
      return <SystemPromptMessage message={message} />
    }
    return (
